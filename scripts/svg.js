...
import { map, filter, downloadAll, mkdirs, remove, exists, assertEquals, walk } from './utils.js';
...
const getStyleDirs = (symbols, dir) => {
    ...
    return Object.fromEntries(styles[type].map((style) => [style, path.resolve(dir, style)]));
};
...
const checkDownloads = async (downloads) => {
    ...
    const missing = await filter(downloads, async ([_, file]) => !(await exists(file)));
